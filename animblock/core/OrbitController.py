...
from mathutils import Matrix, MatrixFactory
...
class OrbitController(Object3D):
    ...
    def __init__(self, input_manager, camera, target=(0, 0, 0), distance=5, 
                initial_azimuth=0, initial_elevation=0, smooth_factor=0.2):
        """
    ...
    def setSensitivity(self, mouse_sensitivity=0.004, zoom_sensitivity=0.1, 
                       pan_sensitivity=0.005, smooth_factor=None):
        """
    ...
    def _handle_mouse_input(self):
        ...
        if not self.dragging:
            if self.input.isMousePressed() == self.MOUSE_ROTATE or self.input.isMousePressed() == self.MOUSE_PAN:
                self.dragging = True
        ...
        if self.dragging and self.last_mouse_pos:
            ...
            if self.input.isMousePressed() == self.MOUSE_ROTATE:
                ...
                self.target_elevation = max(min(self.target_elevation, pi/2 - 0.01), -pi/2 + 0.01)
    ...
    def _update_camera_position(self, immediate=False):
        ...
        self.camera.transform.lookAt(
            ...
            target_with_offset[1], 
            target_with_offset[2]
        )
